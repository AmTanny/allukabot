...
import alluka.modules.sql.notes_sql as sql
from alluka import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from alluka.__main__ import DATA_IMPORT
...
from alluka.modules.rules import get_rules
import alluka.modules.sql.rules_sql as rulessql
from alluka.modules.sql import warns_sql as warnssql
import alluka.modules.sql.blacklist_sql as blacklistsql
from alluka.modules.sql import disable_sql as disabledsql
from alluka.modules.sql import cust_filters_sql as filtersql
import alluka.modules.sql.welcome_sql as welcsql
import alluka.modules.sql.locks_sql as locksql
...
def export_data(bot: Bot, update: Update, chat_data):
    ...
    try:
        ...
        pass
    bot.sendDocument(current_chat_id, document=open('alluka{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This  is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
    os.remove("alluka{}.backup".format(chat_id)) # Cleaning file
