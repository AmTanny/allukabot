...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        message.reply_text(f"Admin {mention_html(user.id, user.first_name)} has banned {mention_html(member.user.id, member.user.first_name)} in " + f"{chat_name}",parse_mode=ParseMode.HTML)
        return log
...
def temp_ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        message.reply_text(f"<b>{html.escape(member.user.first_name)}</b>"+ f" will be banned for {time_val} from "+ f"{chat_name}",parse_mode=ParseMode.HTML)
        return log
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if res:
        bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
        message.reply_text(f"Admin {mention_html(user.id, user.first_name)} has kicked {mention_html(member.user.id, member.user.first_name)} in " + f"{chat_name}",parse_mode=ParseMode.HTML)
...
*Admin only:*
 ...
 - /mute <userhandle>: mute a user. (via handle, or reply)
 - /tmute <userhandle> x(m/h/d): mute a user for x time. (via handle, or reply). m = minutes, h = hours, d = days.
